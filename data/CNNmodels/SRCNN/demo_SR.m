...
filelist_set14=[];
namelist= dir('Set5/*.bmp');
namelist={namelist.name};

for k=1:numel(namelist)
    loadname=strcat('Set5/', char(namelist(k)));
    save_sr_name= strcat('Set5out/', char(namelist(k)),'_sr.mat');
    save_raw_name = strcat('Set5out/', char(namelist(k)),'_raw.mat');
...
im_h= shave(im_h,[up_scale, up_scale ]);
im_gnd= shave(im_gnd,[up_scale, up_scale ]);
im_b= shave(im_b,[up_scale, up_scale ]);
...
image=im_h;
save(save_sr_name,'image' );
image= im_gnd;
save(save_raw_name,'image' );
